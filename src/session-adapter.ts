...
function extractText(blocks: readonly ContentBlock[] | undefined): string {
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'tool-result':
        ...
        break
      default:
        parts.push(`[unsupported block type: ${String((block as { type: string }).type)}]`)
    }
  }
  return parts.join('\n')
}
...
function isReplacement(surfaceOp: unknown): boolean {
  return typeof surfaceOp === 'object' && surfaceOp !== null
}

/** Project one DSH session into the export schema. Never throws on odd input. */
export function adaptSession(session: Session, options: AdaptOptions = {}): SessionExport {
  ...
  for (const event of events) {
    ...
    switch (event.type) {
      ...
      case 'assistant/message': {
        ...
        if (provider || model) lastModel = { provider, model }
        const text: string[] = []
      ...
      case 'tool/result': {
        ...
        const callId = String(
          block?.toolCallId ?? data?.message?.source?.callId ?? '',
        )
      ...
      default: {
        if (typeof event.type === 'string' && SKIPPED_LOG_TYPES.has(event.type)) break
  ...
  const asOfSeq = events.length > 0
    ? Number((events[events.length - 1] as { seq?: unknown }).seq ?? -1)
    : -1
  ...
  const metadata: SessionExportMetadata = {
    ...
    } satisfies SessionExportOmissions,
  }
