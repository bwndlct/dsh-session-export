...
function body(text, placeholder = '_(empty)_') {
...
export function renderMarkdown(export_) {
    const out = ['# DSH Session Export', '', '## Metadata', ''];
    const meta = export_.session;
    const rows = [];
    if (meta.id !== undefined)
        rows.push(['Session ID', meta.id]);
    ...
    if (meta.cwd !== undefined)
        rows.push(['Working Directory', meta.cwd]);
    if (meta.parentSession !== undefined)
        rows.push(['Parent Session', meta.parentSession]);
    if (meta.asOfSeq !== undefined)
        rows.push(['As Of Seq', String(meta.asOfSeq)]);
    if (meta.eventCount !== undefined)
        rows.push(['Exported Events', String(meta.eventCount)]);
    const omitted = meta.omitted;
    if (omitted !== undefined) {
        const parts = [];
        if (omitted.contextMessages)
            parts.push(`${omitted.contextMessages} injected context message(s)`);
        if (omitted.replacedEvents)
            parts.push(`${omitted.replacedEvents} compaction-replaced event(s)`);
        if (omitted.unknownEvents)
            parts.push(`${omitted.unknownEvents} unknown event(s)`);
        if (parts.length > 0)
            rows.push(['Omitted', parts.join(', ')]);
    }
    for (const [label, value] of rows)
        out.push(`- ${label}: ${value}`);
    out.push('', '---', '');
    for (const event of export_.events)
